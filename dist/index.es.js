...
pexprs$j.PExpr.prototype._assertAllApplicationsAreValid = abstract("_assertAllApplicationsAreValid");
...
Trace$2.prototype.recordLRTermination = function(ruleBodyTrace, value) {
  this.terminatingLREntry = new Trace$2(this.input, this.pos, this.pos2, this.expr, false, [value], [ruleBodyTrace]);
...
pexprs$g.Apply.prototype.growSeedResult = function(body, state, origPos, lrMemoRec, newValue) {
  ...
  while (true) {
    ...
    if (state.isTracing()) {
      ...
      lrMemoRec.traceEntry = new Trace$1(state.input, origPos, inputStream.pos, this, true, [newValue], [seedTrace.clone()]);
    }
    inputStream.pos = origPos;
...
Semantics$2.prototype.addOperationOrAttribute = function(type, signature, actionDict) {
  ...
  function doIt(...args) {
    ...
    if (arguments.length !== thisThing.formals.length) {
      throw new Error("Invalid number of arguments passed to " + name2 + " " + type + " (expected " + thisThing.formals.length + ", got " + arguments.length + ")");
    }
...
Semantics$2.prototype.extendOperationOrAttribute = function(type, name2, actionDict) {
  ...
  if (!(this.super && name2 in this.super[typePlural])) {
    throw new Error("Cannot extend " + type + " '" + name2 + "': did not inherit an " + type + " with that name");
  }
...
Semantics$2.prototype.assertNewName = function(name2, type) {
  ...
  if (name2 in this.operations) {
    throw new Error("Cannot add " + type + " '" + name2 + "': an operation with that name already exists");
  }
  if (name2 in this.attributes) {
    throw new Error("Cannot add " + type + " '" + name2 + "': an attribute with that name already exists");
  }
};
...
Semantics$2.createSemantics = function(grammar2, optSuperSemantics) {
  const s = new Semantics$2(grammar2, optSuperSemantics !== void 0 ? optSuperSemantics : Semantics$2.BuiltInSemantics._getSemantics());
...
function Grammar$4(name2, superGrammar, rules, optDefaultStartRule) {
  ...
  if (optDefaultStartRule) {
    if (!(optDefaultStartRule in rules)) {
      throw new Error("Invalid start rule: '" + optDefaultStartRule + "' is not a rule in grammar '" + name2 + "'");
    }
...
Grammar$4.ProtoBuiltInRules = new Grammar$4("ProtoBuiltInRules", void 0, {
  any: {
    body: pexprs$3.any,
    formals: [],
...
GrammarDecl$1.prototype.build = function() {
  const grammar2 = new Grammar$3(this.name, this.ensureSuperGrammar(), this.rules, this.defaultStartRule);
  const grammarErrors = [];
...
function initBuiltInSemantics(builtInRules2) {
  ...
  Semantics.BuiltInSemantics = Semantics.createSemantics(builtInRules2, null).addOperation("asIteration", {
...
function grammars(source, optNamespace) {
  ...
  if (typeof source !== "string") {
    if (isBuffer(source)) {
      ...
      throw new TypeError("Expected string as first argument, got " + common$m.unexpectedObjToString(source));
    }
  }
...
const defaultOperation = {
  ...
  _nonterminal(...children) {
    ...
    const { mapping: mapping2 } = this.args;
    if (!Object.prototype.hasOwnProperty.call(mapping2, ctorName)) {
      if (this.isLexical()) {
        return this.sourceString;
      }
      const realChildren = children.filter((child) => !child.isTerminal());
      if (realChildren.length === 1) {
        return realChildren[0].toAST(mapping2);
      }
    }
    if (typeof mapping2[ctorName] === "number") {
      return children[mapping2[ctorName]].toAST(mapping2);
    }
    const propMap = mapping2[ctorName] || children;
    const node = {
      type: ctorName
    };
    for (const prop in propMap) {
      const mappedProp = mapping2[ctorName] && mapping2[ctorName][prop];
      if (typeof mappedProp === "number") {
        node[prop] = children[mappedProp].toAST(mapping2);
        ...
        if (children[prop] && !children[prop].isTerminal()) {
          node[prop] = children[prop].toAST(mapping2);
        } else {
          delete node[prop];
        }
      }
...
parse$1.List = (iter = {}) => {
  if (iter.ctorName === "Commands")
    return iter.child(0).children.map((c) => parse$1.Command(c));
  if (iter.ctorName === "Values")
    return iter.child(0).children.map((c) => parse$1.Value(c));
  return iter.children.map((c) => parse$1.Value(c));
};
parse$1.KeyValue = (Key = {}, Value = {}) => {
  const key = Key.child(0);
  const value = Value.child(0);
  const keyStr = key.ctorName === "stringLiteral" ? key.sourceString.slice(1, -1) : key.sourceString;
  return { [keyStr]: parse$1.Value(value) };
}, parse$1.Map = (node = {}, nodeIsKeyValues = false) => {
  let res = {};
  const childs = nodeIsKeyValues ? node.children : innerChilds(node);
  for (let c of childs)
    res = __spreadValues(__spreadValues({}, res), parse$1.KeyValue(c.child(0), c.child(2)));
  return res;
};
parse$1.Value = (node = {}) => {
  switch (node.ctorName) {
    case "literal":
      return parse$1.literal(node);
    case "Value":
      return parse$1.Value(node.child(0));
    case "identifier":
      return String(node.sourceString);
    case "List":
      return parse$1.List(node.child(1));
    case "KeyValue":
...
parse$1.Command = (Command = {}) => {
  ...
  for (let valuesIter of Values.child(0).children) {
    ...
    switch (Value.ctorName) {
      case "List":
        res.push(innerChilds(Value).map((c) => parse$1.Value(c)));
        break;
      case "Map": {
        res.push(parse$1.Map(Value));
        break;
      }
      default: {
        res.push(parse$1.Value(Value));
        break;
      }
    }
  }
...
const mapping = {
  KeyValues: (KeyValues, _comma) => {
    return {
      type: "Map",
      value: parse$1.Map(KeyValues, true)
    };
  },
  markdown: (_0, node, _1) => {
    return {
...
parser.parse = (userText, g, options = {}) => {
  ...
  for (let o of mappedAST) {
    if (o.type === "Markdown")
      ...
      result = __spreadValues(__spreadValues({}, result), o.value);
  }
  const out = { result };
  if (outputAST)
    out.AST = toAST(match);
...
const parse = (userText = "", options = {}) => {
  const grammarText = options.ohmGrammar || path.readFileSync(path.join(__dirname, "mationGrammar.ohm"));
